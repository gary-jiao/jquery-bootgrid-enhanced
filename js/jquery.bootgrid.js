...
;(function ($, window, undefined)
{
    ...
    */
    ...
    function loadData()
    {
        ...
        function initGroupSummary(groupSummaries, groupField) {
            groupSummaries[groupField] = {};
            if (isArrayFn(that.options.groupingView.calculateFields)) {
                $.each(that.options.groupingView.calculateFields, function(idx, item) {
                    groupSummaries[groupField][item] = 0.00;
                });
            } else {
                groupSummaries[groupField][that.options.groupingView.calculateFields] = 0.00;
            }
        }
        ...
        if (this.options.ajax)
        {
            ...
            var settings = {
                ...
                success: function(response)
                {
                    ...
                    if (that.options.grouping) {
                        //if grouping feature is enabled then update the return result and insert group summary to list
                        var groupOpts = that.options.groupingView;
                        var groupFieldValue = '';
                        var groupSummaries = {};
                        ...
                        $.each(response.rows, function (index, row) {
                            ...
                            if (isArrayFn(groupOpts.calculateFields)) {
                                $.each(groupOpts.calculateFields, function(idx, item) {
                                    groupSummaries[gpFdValue][item] += parseFloat(row[item]);
                                });
                            } else {
                                groupSummaries[gpFdValue][groupOpts.calculateFields] += parseFloat(row[groupOpts.calculateFields]);
                            }
                            row[groupOpts.groupField] = '';
			...
			if (this.options.jsonData == undefined)	{
                ...
                var tmp = $.mockjax({
                    ...
                    responseText: this.options.jsonData
                });
                this.xqr = $.ajax(settings);
                $.mockjax.clear(tmp);
    ...
    function renderRows(rows)
    {
        if (rows.length > 0)
        {
            ...
            $.each(rows, function (index, row)
            {
                ...
                if (row.groupHeader) {
                    html += tpl.groupHeader.resolve(getParams.call(that, { columns: that.columns.length, value: row.groupHeader }));
                } else {
                    ...
                    if (row.isSummary) {
                        rowAttr += " class=\"group-summary\"";
                    } else if (rowCss.length > 0) {
        ...
        renderGroupAndSummary.call(this, tbody, rows);
    }

    function renderGroupAndSummary(tbody, rows) {
    ...
    Grid.defaults = {
        ...
        templates: {
            ...
            groupHeader: "<tr><td colspan=\"{{ctx.columns}}\" class=\"group-header\">{{ctx.value}}</td></tr>"
        }
    };
